...
2. **`scripts/pause-awake.sh`** — fires on the `Notification` hook (Claude needs a permission, or has been idle waiting for input ≥60s), and on `PreToolUse` matched against `AskUserQuestion`. `AskUserQuestion` blocks waiting for the user but (unlike permission prompts / plan approval) emits no `Notification` event, so the `PreToolUse` matcher in `hooks.json` is the only hook that can pause it; it's declared *after* the catch-all `keep-awake.sh` so the pause marker `pause-awake.sh` writes wins. Writes `~/.claude/keep-awake-state/paused/<cli-pid>`, marking that this session is *not* working. The daemon stops preventing sleep only when **every** live session is paused; any session still working keeps the Mac awake. The next `keep-awake.sh` hook (resume) removes the marker.
