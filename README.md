...
1. **`scripts/keep-awake.sh`** — fires on every `UserPromptSubmit`, `PreToolUse`, and `PostToolUse` hook. Registers the current session in `~/.claude/keep-awake-state/sessions/<cli-pid>`, clears any pause marker for it, and starts a daemon if none is running. Exception: a `PreToolUse` for `AskUserQuestion` delegates to `pause-awake.sh` instead of clearing the marker — that tool blocks waiting for the user but (unlike permission prompts / plan approval) emits no `Notification` event, so this is the only hook that can pause it; the `PostToolUse` after the answer resumes normally. Sessions are keyed by the Claude CLI process PID (stable for the process lifetime), not `session_id` (which changes on `/clear`, `/compact`, resume — UUID keying leaked one file per session_id that `Stop` never reaped).
2. **`scripts/pause-awake.sh`** — fires on the `Notification` hook (Claude needs a permission, or has been idle waiting for input ≥60s), and is also invoked by `keep-awake.sh` on a `PreToolUse` for `AskUserQuestion`. Writes `~/.claude/keep-awake-state/paused/<cli-pid>`, marking that this session is *not* working. The daemon stops preventing sleep only when **every** live session is paused; any session still working keeps the Mac awake. The next `keep-awake.sh` hook (resume) removes the marker.
