...
- **Releases while Claude waits for you** — a session blocked on a permission prompt, `AskUserQuestion`, plan approval, or a turn you interrupted with `Esc` stops requesting wake (per-session: other working sessions still keep the Mac awake)
...
**Interrupted turns**: pressing `Esc` aborts a turn but fires *no* `Stop` and *no* `Notification` — so the session would stay registered and unpaused and pin the Mac until the 2 h watchdog. Claude Code records the abort in the session transcript as a final `[Request interrupted by user]` (or `…for tool use]`) message, so `keep-awake.sh` stores each session's transcript path in `~/.claude/keep-awake-state/transcripts/<cli-pid>`, and the daemon's 1 Hz poll treats a session as idle when its transcript's last message is that marker — releasing within ~1 s. A genuinely running foreground tool is never mistaken for this: its `tool_use` entry is written at tool *start*, so a live tool leaves a trailing `tool_use` with no result (never the interrupt marker) and the Mac stays awake. The next prompt re-registers the session.
...
│   └── ...
├── transcripts/
│   ├── <cli-pid>           # path to that session's transcript (daemon reads its tail for Esc interrupts)
│   └── ...
